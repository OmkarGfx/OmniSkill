...
webact inserttext <text>       # Insert text in one shot (fast, no formatting)
webact paste <text>            # Paste via clipboard event
webact clipboard --html <html> # Paste rich HTML via real clipboard (Google Docs, Notion)
webact fill <fields_json>      # Fill multiple form fields at once
...
webact resolve <sel>           # Get link/form target URL without clicking
webact download <url> [path]   # Download a file from URL
webact network capture [secs]  # Capture XHR/fetch requests
webact zoom [level]            # Zoom page (in, out, 50, reset)
webact lock                    # Lock session to prevent concurrent access
webact unlock                  # Unlock session
...
webact install                 # Install webact for a specific MCP client
webact config [key] [value]    # Get or set webact configuration
