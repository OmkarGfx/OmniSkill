...
Use this note for future WebAct feedback triage and status updates. Do not rediscover the DB path each time.

## Source of Truth

- Website + API: `~/src/webact/www` (Vercel serverless functions)
- Previous location: `vm-sites/webact-api` (deprecated Express app)
- DB name: `webact`
- Collection: `feedback`
...
```bash
set -a
. /Users/karthik/src/vm-sites/env/webact-api.env.sh
set +a
```
...
const dbx = db.getSiblingDB("webact");
printjson(
  dbx.feedback
