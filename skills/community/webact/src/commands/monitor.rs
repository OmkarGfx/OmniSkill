...
/// Global timestamp of the last webact tool call (epoch ms).
...
fn deliver_notification(method: &DeliveryMethod, message: &str) -> Result<()> {
    match method {
        DeliveryMethod::Socket(path) => {
            ipc::ipc_send_message(path, message, "webact-monitor")
        }
        DeliveryMethod::DirectPaste(pane) => {
            let formatted = format!("[from webact-monitor]: {message}");
...
/// Reports via `__webactFaviconChanged` binding with JSON: {"old":"...","new":"..."}
const FAVICON_OBSERVER_JS: &str = r#"
(() => {
    if (window.__webactFaviconObserver) return;
    ...
    const observer = new MutationObserver(() => {
        ...
        if (cur !== lastHref) {
            const old = lastHref;
            lastHref = cur;
            try { __webactFaviconChanged(JSON.stringify({old, new: cur})); } catch(e) {}
        }
    });
    ...
    window.__webactFaviconObserver = observer;
})();
"#;
...
async fn inject_favicon_observer(
    ...
    if let Err(e) = cdp
        .send_to_session(
            "Runtime.addBinding",
            json!({"name": "__webactFaviconChanged"}),
            &session_id,
        )
        .await
    {
        ...
        match method {
            ...
            "Runtime.bindingCalled" => {
                let name = event
                    ...
                    .unwrap_or_default();
                if name != "__webactFaviconChanged" {
                    continue;
                }
