...
pub(super) async fn cmd_new_tab(ctx: &mut AppContext, url: Option<&str>) -> Result<()> {
    let mut state = ctx.load_session_state()?;
    // Prune stale tab IDs (tabs closed outside webact) before checking limit.
...
pub(super) async fn cmd_uninstall(_ctx: &mut AppContext) -> Result<()> {
    ...
    // Clean up data directory (~/.webact/)
    ...
    // Clean up config directory (~/.config/webact/)
    ...
    if let Ok(entries) = std::fs::read_dir("/tmp") {
        for entry in entries.flatten() {
            ...
            if name.starts_with("webact-") && name.ends_with(".sock") {
