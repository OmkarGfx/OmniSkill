...
/// Structured warning log to stderr. Prefixed with "webact:" for grepability.
#[macro_export]
macro_rules! wlog {
    ($($arg:tt)*) => {{
        eprintln!("webact: {}", format!($($arg)*));
    }};
}
...
impl Drop for CdpClient {
    fn drop(&mut self) {
        if !self.closed && std::env::var_os("WEBACT_DEBUG").is_some() {
            eprintln!("webact: CdpClient dropped without close()");
        }
    }
}
