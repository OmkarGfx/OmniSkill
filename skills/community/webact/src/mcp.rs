...
pub async fn run_mcp_server() -> Result<()> {
    ...
    // Auto-register on the webact bus and start IPC socket
    let mut bus_state = bus::WebactBusState::new();
    bus_state.do_register(None);

    let ipc_socket_path = if let (Some(name), Some(pane_display), Some(session)) =
        (bus_state.name.as_deref(), bus_state.pane.as_deref(), bus_state.channel.as_deref())
    {
        if let Some(pane_info) = ipc::detect_tmux_pane() {
            match ipc::start_socket_listener(
                ...
                Ok(path) => Some(path),
                Err(e) => {
                    eprintln!("webact ipc: socket failed: {e}");
                    None
                }
            }
        } else {
    ...
    if let Some(path) = &ipc_socket_path {
        ...
        eprintln!("webact ipc: socket removed");
    }
...
async fn handle_tool_call(
    ctx: &mut AppContext,
    bus_state: &mut bus::WebactBusState,
    tool_name: &str,
    arguments: &Value,
) -> Result<Vec<Value>> {
    ...
    let no_browser = matches!(
        ...
        | "who" | "send_message" | "signal_done" | "register" | "unregister"
