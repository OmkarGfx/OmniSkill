//! Webact bus — registration, message tracking, and coordination.
//!
//! Provides agent identity management, pending message tracking, nudge
//! timers, broadcast, and signal_done. Built on top of the ipc module's
...
const NICKNAMES: &[&str] = &[
    "ace", "arc", "ash", "atom", "aura", "axle", "base", "beam", "beta", "blip", "bolt", "buzz",
    "byte", "calm", "chip", "claw", "clip", "coal", "coil", "cold", "core", "cove", "crow", "cube",
    "dart", "dash", "dawn", "dew", "disc", "dock", "dome", "dove", "dune", "dusk", "echo", "edge",
    "elm", "ember", "fang", "fern", "fizz", "flap", "flip", "flux", "foam", "fuse", "gale", "gate",
    "glow", "grid", "gust", "halo", "haze", "helm", "hex", "hive", "hook", "hum", "iris", "jade",
    "jazz", "keel", "knot", "lark", "leaf", "lens", "lime", "link", "loom", "loop", "lynx", "mace",
    "mesa", "mint", "mist", "moth", "nest", "node", "nova", "onyx", "opal", "palm", "peak", "pine",
    "plum", "pond", "puck", "pulse", "quay", "rain", "reef", "rune", "sage", "silk", "silo",
    "snap", "spar", "tarn", "vex", "wren",
];
...
fn pick_nickname() -> String {
    ...
    available.first().unwrap_or(&"agent").to_string()
}
...
fn set_pending(recipient_pane: &str, msg_id: &str, envelope: &Value) {
    let key = format!("@webact-pending-{msg_id}");
...
fn read_pending(pane: &str) -> Vec<(String, Value)> {
    ...
    for line in text.lines() {
        if let Some(rest) = line.strip_prefix("@webact-pending-") {
...
pub fn pending_warnings(pane: &str) -> Option<String> {
    ...
    Some(format!(
        "\u{26a0} You have {} unanswered message(s). Respond using signal_done or send_message with reply_to:\n{}",
        warnings.len(),
        warnings.join("\n")
    ))
}
...
fn spawn_nudge_timer(
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            if snap1 != snap2 {
                eprintln!("webact bus: nudge for {msg_id}: recipient still busy, resetting timer");
            ...
            nudge_count += 1;
            eprintln!(
                "webact bus: nudging recipient for {msg_id} (attempt {nudge_count}/{NUDGE_MAX})"
            );

            let nudge_msg = format!(
                "[webact] You have an unanswered request from {sender_name}. \
                 Reply using send_message or signal_done with reply_to: \"{msg_id}\""
            );
            let _ = ipc::send_to_pane(&recipient_pane, &nudge_msg);

            if nudge_count >= NUDGE_MAX {
                eprintln!("webact bus: gave up nudging for {msg_id} after {NUDGE_MAX} attempts");
                return;
            }
...
pub struct WebactBusState {
    pub name: Option<String>,
    pub nick: Option<String>,
    pub pane: Option<String>, // display pane ID
...
impl WebactBusState {
    ...
    pub fn unregister(&mut self) {
        ...
        if let Some(pane) = &self.pane {
            ...
            if let Some(name) = &self.name {
                eprintln!("webact bus: unregistered \"{name}\"");
            }
        }
        self.name = None;
        self.nick = None;
    ...
    pub fn do_register(&mut self, custom_name: Option<&str>) {
        ...
        eprintln!(
            "webact bus: registered as \"{name}\" aka \"{nick}\" on channel \"{session}\" (pane {pane})"
        );
        self.name = Some(name);
        self.nick = Some(nick);
    }
...
pub fn cmd_who(state: &WebactBusState, ctx: &mut AppContext, show_all: bool) -> Result<()> {
    if show_all {
...
pub fn cmd_signal_done(
    ...
    let my_name = state.name.as_deref().unwrap_or("unknown").to_string();
    let msg_id = gen_msg_id();
    let message = format!("[from {my_name}]: {summary} Request: {request} [msg_id: {msg_id}]");
...
pub fn cmd_unregister(state: &mut WebactBusState, ctx: &mut AppContext) -> Result<()> {
